...
- 2026-06-21: Batched the skip-existing CAS chunk existence checks. The writer now prepares the blob layout first, performs one key-only `PointReadPlan::from_unique_keys` lookup per file/blob for transaction-unique chunk keys, and then stages only missing chunk payload rows. The profiler also reports `*_binary_cas_chunk_lookup_batch_count` so lookup keys and backend batches are visible separately.
- 2026-06-21: Reran the Downloads sample at FastCDC 256/1024/4096 KiB with RocksDB 0.24 / librocksdb 10.4.2. Median of 3 runs:

| Backend | Cold median | Warm median | Lookup keys | Lookup batches | Lookup hits | Lookup misses | Lookup time | Lookup share of cold | `.lix` total | SST | Blob |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| rocksdb | 7508 ms | 4918 ms | 1,757 | 589 | 12 | 1,745 | 416 ms | 5.6% | 1.603 GB | 1.602 GB | 0 B |
| rocksdb-blob 16 KiB | 6503 ms | 3838 ms | 1,757 | 589 | 12 | 1,745 | 6.8 ms | 0.1% | 1.602 GB | 2.4 MB | 1.600 GB |
...
- 2026-06-21: Added `--read-bench` to `profile_fs_open`. It reuses the imported/warm workspace and measures Lix-level `lix_file.data` reads for all files, the four largest files twice, and a deterministic 16-file small-file point-read sample. The 16-file sample replaced an initial 128-file point-read sample because 128 separate SQL point reads took about 71-75 seconds across backends and mostly measured per-query SQL overhead rather than BlobDB storage behavior.
- 2026-06-21: Ran the read benchmark matrix on the sanitized Downloads sample using FastCDC 256/1024/4096 KiB, with 3 runs per backend. Median results:
...
- 2026-06-21: Read benchmark interpretation: BlobDB does not show a meaningful read regression on this corpus. Full-corpus reads are essentially tied at about 0.8 seconds. The four-largest-file read is noisy but BlobDB 32 KiB was fastest in the median, and BlobDB 16 KiB was modestly slower than plain RocksDB. The 16 small-file point reads are slow for all backends, around 8-9 seconds total, which points at per-query SQL/read-path overhead rather than BlobDB indirection. This shifts the candidate default slightly toward BlobDB 32 KiB plus FastCDC 256/1024/4096 KiB, pending a SQLite baseline.
- 2026-06-21: Verification passed after adding `--read-bench`: smoke read benchmark on a tiny 3-file corpus, `cargo check -p lix_sdk --features sqlite,rocksdb --example profile_fs_open` with the local libclang env, `cargo build --release -p lix_sdk --features sqlite,rocksdb --example profile_fs_open` with the local libclang env, and `cargo check -p lix_sdk --features sqlite --example profile_fs_open`.
- 2026-06-21: Ran the direct SQLite-current vs BlobDB-candidate comparison on the sanitized Downloads sample with 3 runs per backend. SQLite used current/default FastCDC 16/64/256 KiB. BlobDB used BlobDB min 32 KiB plus FastCDC 256/1024/4096 KiB.

| Backend/config | Cold | Warm | Read all files | Read 4 largest | Repeat 4 largest | Read 16 small | `.lix` total | Main DB/SST | Blob | CAS chunks | CAS refs |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| sqlite current 16/64/256 | 17232 ms | 11506 ms | 3231 ms | 12570 ms | 12230 ms | 50196 ms | 1.636 GB | 1.636 GB | 0 B | 20,223 | 20,299 |
