...
test("launching with multiple workspace args creates independent windows", async ({
	...
	try {
		...
		await ensureFilesHomeTabActive(firstPage);
		...
		await ensureFilesHomeTabActive(secondPage);
...
test("relaunch reopens saved directory and transient file workspaces", async ({
	...
	try {
		...
		await expectWindowCount(electronApp, 2);
		await ensureFilesHomeTabActive(directoryPage);
		await expect(
...
test("relaunch restores the last document opened in a transient directory", async ({
	...
	try {
		...
		// Tabs keep the auto-opened document open beside the newly opened one;
		// the active document is listed first.
...
test("closed workspace windows are removed from the relaunch session", async ({
	...
	try {
		...
		await ensureFilesHomeTabActive(restoredPage);
...
test("macOS open-file events create workspace windows for folders and files", async ({
	...
	try {
		...
		await ensureFilesHomeTabActive(folderPage);
...
test("macOS open-file events open standalone files as transient workspaces", async ({
	...
	try {
		...
		// The file tree lives in the pinned Files home tab now, not the left panel.
		await ensureFilesHomeTabActive(filePage);
...
test("launching with multiple standalone markdown files creates one grouped transient workspace", async ({
	...
	try {
		...
		// The file tree lives in the pinned Files home tab now, not the left panel.
		await ensureFilesHomeTabActive(groupedPage);
...
test("mixed folder and standalone markdown args create folder and grouped file windows", async ({
	...
	try {
		...
		// The file tree lives in the pinned Files home tab now, not the left panel.
		await ensureFilesHomeTabActive(groupedFilePage);
		await expect(
