...
test("close preserves other panels and selects the remaining tab", async () => {
	...
	expect(store.getSnapshot()?.panels.central.activeInstance).toBe("a");
	expect(store.getSnapshot()?.panels.central.views).toHaveLength(1);
	expect(store.getSnapshot()?.panels.left.activeInstance).toBe("files");
});
...
test("repeated external open activates a document after an internal tab switch", async () => {
	...
	store.setSnapshot({
		...
		panels: {
			...state.panels,
			central: { ...state.panels.central, activeInstance: "b" },
		},
	});
	await commands.open("/a.md");
	expect(store.getSnapshot()?.panels.central.activeInstance).toBe("a");
